...
"""
A python script which utilises a complex vector class to perform vector calculus
and test if Hansen's vector's hold for various points in space.
"""
...
#Placing all task 2 values in a table for easier viewing, using table specifications from python website
...
def partialx(v, x, y, z):
    """
    Partial derivative functions for differentating a vector v with respect to x
    """
    return (v(x+H, y, z) - v(x-H, y, z)) * (1/(2*H))    #Need to call y and z to keep them fixed because we're in 3D.

def partialy(v, x, y, z):
    """
    Partial derivative functions for differentating a vector v with respect to y
    """
    return (v(x, y+H, z) - v(x, y-H, z)) * (1/(2*H))

def partialz(v, x, y, z):
    """
    Partial derivative functions for differentating a vector v with respect to z
    """
...
def divergence(v, x, y, z):
    """
    Function for finding the divergence of a vector v with respect to x, y and z
    """
...
def curl(v, x, y, z):
    """
    Function for finding the curl of a vector v with respect to x, y and z
    """
    del_v_dx = partialx(v, x, y, z)
    del_v_dy = partialy(v, x, y, z)
    del_v_dz = partialz(v, x ,y, z)
